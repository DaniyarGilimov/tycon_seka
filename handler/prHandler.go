...
// GetPPRHandler gives business's pr
func GetPPRHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := localcontroller.GetPPR(BID, database)

	if err != nil {
		res.Message = "Error Get PR page. Not Found"
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(res)
		return
	}
...
// UpgradePRHandler allows to upgrade PR
func UpgradePRHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	err = controller.UpgradePR(upgradeRegist, business, user, database)
